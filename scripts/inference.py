#!/usr/bin/env python3
import sys, os
# make sure to import the installed ikflow, not any local folder named "ikflow"
sys.path.pop(0)
...
def main():
    ...
    # include tag suffix to match actual folder name
    suffix = f"_{DATASET_TAG_NON_SELF_COLLIDING}"
    ddir = os.path.join(DATASET_DIR, f"{robot.name}{suffix}")
    samples_fp, poses_fp, *_ = get_dataset_filepaths(
        ddir, [DATASET_TAG_NON_SELF_COLLIDING]
    )
    ...
    # poses stored as (x,y,z, qx,qy,qz,qw) → reorder → (x,y,z,qw,qx,qy,qz)
